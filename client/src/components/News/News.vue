<template>
    <div class="box">
        ...
        <div v-else>
            ...
            <ul class="article-info-list">
                <li><i>Date: {{getTime(article)}}</i></li>
                <br>
                <li>{{article.text}}</li>
            </ul>
        </div>
    ...
    export default {
        ...
        methods: {
            getTime: (article) => {
                ...
                const timeString = `${days[time.getDay()]}, ${time.getDate().toLocaleString('en-US', {minimumIntegerDigits: 2})} ${toMonthName(time.getMonth() + 1)} ${time.getHours().toLocaleString('en-US', {minimumIntegerDigits: 2})}${time.getMinutes().toLocaleString('en-US', {minimumIntegerDigits: 2})}z`;
        ...
        computed: () => {
            return {
                ...
                imageURL: `/api/file/${this.event.image}`,
            }
        }
    }}
</script>
